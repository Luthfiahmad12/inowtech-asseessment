    ...
    <div>
                                ...
                                @foreach ($students as $student)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 text-sm font-medium text-gray-800 whitespace-nowrap">
                                            {{ $student->address }}</td>
                                        <td class="px-6 py-4 text-sm font-medium text-gray-800 whitespace-nowrap">
                                            {{ $student->classroom->class_name }}</td>
                                        <td class="px-6 py-4 text-sm font-medium text-gray-800 whitespace-nowrap">
                                            <div class="flex justify-end space-x-2">
                                                <x-primary-button wire:click="getData({{ $student->id }})">
                                                    Edit
                                                </x-primary-button>
                                                <x-danger-button wire:click="destroy({{ $student->id }})">
                                                    Hapus
                                                </x-danger-button>
                                                <div wire:loading wire:target="getData({{ $student->id }})">
                                                ...
                                                <div wire:loading wire:target="destroy({{ $student->id }})">
                                                    <div class="animate-spin inline-block size-6 border-[3px] border-current border-t-transparent text-blue-600 rounded-full"
                                                        role="status" aria-label="loading">
                                                        <span class="sr-only">Loading...</span>
                                                    </div>
        ...
        <x-modal name="create-data" :show="$errors->isNotEmpty()" focusable>
            <form wire:submit="save" class="p-6">
                ...
                <div>
                    <div class="sm:col-span-3">
                        <label for="address" class="inline-block text-lg font-medium text-gray-900 mt-2.5">
                            Alamat
                        </label>
                    </div>
                    <div class="sm:col-span-9">
                        <x-text-input id="address" wire:model="address" class="block w-full mt-1" type="text"
        ...
        <x-modal name="update-data" :show="$errors->isNotEmpty()" focusable>
            <form wire:submit="edit" class="p-6">
                <div>
                    <div class="sm:col-span-3">
                        <label for="edit_name" class="inline-block text-lg font-medium text-gray-900 mt-2.5">
                            Nama Siswa
                        </label>
                    </div>
                    <div class="sm:col-span-9">
                ...
                <div>
                    <div class="sm:col-span-3">
                        <label for="edit_address" class="inline-block text-lg font-medium text-gray-900 mt-2.5">
                            Alamat
                        </label>
                    </div>
                    <div class="sm:col-span-9">
                        <x-text-input id="edit_address" wire:model="edit_address" class="block w-full mt-1"
                            type="text" :value="old('edit_address')" autofocus />
                        <x-input-error :messages="$errors->get('edit_address')" class="mt-2" />
                    </div>
                </div>
                <div>
                    <div class="sm:col-span-3">
                        <label for="edit_classroom_id" class="inline-block text-lg font-medium text-gray-900 mt-2.5">
                            Pilih Kelas
                        </label>
                    </div>
                    <select name="edit_classroom_id" wire:model="edit_classroom_id"
                        ...
                        @foreach ($classrooms as $classroom)
                            <option value="{{ $classroom->id }}"
                                {{ $classroom->id == $classroom_id ? 'selected' : '' }}>
